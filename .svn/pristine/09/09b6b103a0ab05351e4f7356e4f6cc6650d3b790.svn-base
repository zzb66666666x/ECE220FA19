...
read_graph (const char* fname)
{
	...
	//allocate memory for graph_t g here
        perror ("malloc graph");
		fclose (f);
		return NULL;
    }
...
{
    heap_t* h;
	//use *h here because sizeof needs the real size of the heap_t rather than a pointer
	//the heap take the memory space but uninitialized
    if (NULL == (h = malloc (sizeof (*h))) ||
        NULL == (h->elt = malloc (g->n_vertices * sizeof (*h->elt)))) {
	//Here, the h->elt is a pointer, then use the * to tell the sizeof that we want to give some space for an array of integers
	if (NULL != h) {
	    free (h);
	}
        return NULL;
...
make_pyramid_tree (graph_t* g)
{
	...
	//the space for pyramid tree was allocated here
	free (pos);
	if (NULL != p) {
	    free (p);
	}
...
//Below are the helper functions that I may need.
...
main (int argc, const char* const argv[])
{
	...
	//Build graph and build the pyramid tree
	...
	//Initialize the heap memory for graph
    if (NULL == (h = make_heap_for_graph (g))) {
		free_graph (g);
		return 3;
    }
	//Initialize the pyramid tree
	...
	//When the source or the destination is empty:
	//When the path is longer than the max value:
        printf ("Requests not matched.\n");
    } else {
	//Succeed? Plot it.
	show_find_results (g, &r1, &r2, &src_vs, &dst_vs, &path);
    }
	//Call the functions defined above to free heap memories.
    free_pyramid_tree (p);
    free_heap (h);
    free_graph (g);
